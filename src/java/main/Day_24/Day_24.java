...
class Day_24 {
    public static void main(String[] args) throws Exception {
        log(Day_24.class.getName());
        final List<String> input = loadTextFileAsList(Day_24.class, "input.txt");
        Map<Pair<Double, Double>, Boolean> tiles = new HashMap<>();
        for (var line : input) {
            double x = 0;
            double y = 0;
            while (line.length() > 0) {
                if (stringChunkExistsAtStart(line, "e")) {
                    x += 1;
                    line = stringRemoveStartChunk(line, "e");
                } else if (stringChunkExistsAtStart(line, "w")) {
                    x -= 1;
                    line = stringRemoveStartChunk(line, "w");
                } else if (stringChunkExistsAtStart(line, "se")) {
                    x += 0.5;
                    y -= 0.5;
                    line = stringRemoveStartChunk(line, "se");
                } else if (stringChunkExistsAtStart(line, "sw")) {
                    x -= 0.5;
                    y -= 0.5;
                    line = stringRemoveStartChunk(line, "sw");
                } else if (stringChunkExistsAtStart(line, "ne")) {
                    x += 0.5;
                    y += 0.5;
                    line = stringRemoveStartChunk(line, "ne");
                } else if (stringChunkExistsAtStart(line, "nw")) {
                    x -= 0.5;
                    y += 0.5;
                    line = stringRemoveStartChunk(line, "nw");
                }
            }
            // Now set this tile

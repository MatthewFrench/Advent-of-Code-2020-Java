...
class Day_20 {
    public static void main(String[] args) throws Exception {
        log(Day_20.class.getName());
        final String input = loadTextFile(Day_20.class, "input.txt");
        final List<String> rawTiles = splitString(input, "\n\n");
        ...
        log("Bounds- x: " + bounds.getValue0().getValue0() + " to " + bounds.getValue0().getValue1() + ", y: "
        ...
        for (var tileX = boundsMinX; tileX <= boundsMaxX; tileX++) {
            ...
            for (var tileY = boundsMinY; tileY <= boundsMaxY; tileY++) {
                ...
                log("Putting in image index: " + tile.index);
        ...
        for (var comparisonImage : combinedComparisonImages) {
            ...
            if (seaMonsterParts > 0) {
                log("Found sea monster parts: " + seaMonsterParts);
                var totalTrue = countTrue(comparisonImage,  combinedImageWidth, combinedImageHeight);
                log("Rough waters that are not sea monster: " + (totalTrue - seaMonsterParts));
            }
        }
    }
    ...
    static Triplet<Pair<Tile, Integer>, Integer, Integer> getTileNearPosition(Map<Pair<Integer, Integer>, Pair<Tile, Integer>> tilePlacement, int x, int y) {
        var positions = List(Pair.with(x - 1, y), Pair.with(x + 1, y), Pair.with(x, y - 1), Pair.with(x, y + 1));
    ...
    static class Tile {
        ...
        Tile(String rawTile) {
            var split = splitString(rawTile, ":\n");
            index = Integer.parseInt(stringRemoveStartChunk(split.get(0), "Tile "));
